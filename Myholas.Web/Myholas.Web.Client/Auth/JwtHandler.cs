    ...
    public class JwtHandler : DelegatingHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            if (response.StatusCode == System.Net.HttpStatusCode.Unauthorized) // 401
            {
                ...
                // если пришли с логина
                if (request.RequestUri?.AbsolutePath.Contains("login", StringComparison.OrdinalIgnoreCase) == true)
                {
                    return response; // возвращаем 401 обратно в Login
                }
                // Очищаем токен
